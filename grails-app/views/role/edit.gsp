...
<html>
    ...
    <body>
        <div class="nav">
	    <h1 class="heading1">Edit Role</h1>
		  ...
		  <fieldset>
			...
			<g:each var="c" in="${grailsApplication.controllerClasses.sort{it.name}}">   
			     <g:if test="${!exceptedControllers.contains(c.name)}">
					...
					<g:each var="action" in="${actions.unique().sort()}">
					    <%
						boolean checked = false
						roleInstance.permissions.each { permission ->
						    if (permission.controller.equals(c.name.toLowerCase()) &&
							permission.action.equals(action)) {
							checked = true
						    }
						}
					    %>
					    <li><g:checkBox name="${controllerName}" value="${action}" checked="${checked}"/>${GrailsNameUtils.getNaturalName(action)}</li>
					</g:each>
					</ul></div>
					</td>
				    </tr>
